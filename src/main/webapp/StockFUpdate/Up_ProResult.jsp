...
$(document).ready(function(){
	...
	$('.SearBtn').click(function(){
		...
		if(!pass){
			...
			$.ajax({
				url : '${contextPath}/InfoLoading/StepResult.do',
				type : 'POST',
			            ...
			            if(data.List.length > 0) {
			            	for(var i = 0; i < data.List.length; i++){
			            	    var row = '<tr>' +
			            	        ...
			            	        '<td>' + (data.List[i].endMth ?? 'N/A') + '</td>' +
			            	        '<td>' + (data.List[i].stepcd ?? 'N/A') + '</td>' +
			            	        '<td>' + (data.List[i].step ?? 'N/A') + '</td>' +
			            	        '<td>' + (data.List[i].iotype ?? 'N/A') + '</td>' +
			            	        '<td>' + (data.List[i].pono ?? 'N/A') + '</td>' +
			            	        '<td>' + (data.List[i].ponotype ?? 'N/A') + '</td>' +
